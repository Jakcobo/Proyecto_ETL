...
def rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Renombra columnas quitando espacios y caracteres especiales."""
    logger.info("Renaming columns...")
    original_cols = df.columns.tolist()
    df.columns = df.columns.str.replace(' ', '_', regex=False)
    # Podrías añadir más limpieza aquí si es necesario (ej. a minúsculas, quitar caracteres especiales)
...
def drop_unnecessary_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    columns_to_drop = [
        'host_name',          # Eliminada porque ya tenemos host_id
        'country_code',       # Redundante si tenemos 'country' o si no se usa
        'country',            # Podría ser redundante o innecesaria dependiendo del análisis
        'house_rules',        # Texto largo, puede no ser útil para análisis cuantitativo
        'license'             # Columna mayormente vacía o no estandarizada
    ]
