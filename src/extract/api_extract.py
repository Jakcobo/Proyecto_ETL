# /home/nicolas/Escritorio/proyecto ETL/develop/src/extract/api_extract.py
...
# Es buena práctica configurar el logger a nivel de módulo,
# y si el script se ejecuta como __main__, configurar un handler básico.
# Airflow manejará la configuración del logging cuando se ejecute como parte de un DAG.
logger = logging.getLogger(__name__)

# Constante para la ruta relativa del archivo desde la raíz del proyecto
# Esto hace que la función sea más robusta si la raíz del proyecto cambia
# o si se llama desde diferentes contextos (como Airflow).
RELATIVE_CSV_PATH_API = "data/raw/ny_places_foursquare_v3.csv"

def get_project_root() -> str:
    """
    Encuentra la ruta raíz del proyecto asumiendo que este script está en
    PROJECT_ROOT/src/extract/api_extract.py.
    """
    # Ruta actual del archivo: .../src/extract/api_extract.py
    # Queremos llegar a .../
    # os.path.dirname(__file__) -> .../src/extract
    # os.path.join(..., '..')   -> .../src
    # os.path.join(..., '..')   -> .../ (raíz del proyecto)
    return os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))

def extract_foursquare_data(csv_path: str = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        # Re-lanzar para que la tarea de Airflow falle y lo registre correctamente.
        raise
    except Exception as e:
        logger.error(f"Error extrayendo datos de API (Foursquare) desde CSV '{absolute_file_path}': {e}", exc_info=True)
        # Re-lanzar para que la tarea de Airflow falle.
        raise

if __name__ == '__main__':
    # Este bloque se ejecuta solo si ejecutas este script directamente (ej. python src/extract/api_extract.py)
    # Es útil para pruebas rápidas del módulo.
    ...
    try:
        ...
        api_df_default = extract_foursquare_data()
        ...
        # Prueba 2: Proporcionando una ruta (puedes cambiarla a una ruta válida en tu sistema para probar)
        # Si el archivo está donde se espera, esta prueba debería funcionar igual que la anterior.
